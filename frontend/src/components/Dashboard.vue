<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex items-center gap-2">
          <button @click="toggleLanguage" class="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">
            {{ locale === 'en' ? '中文' : 'English' }}
          </button>
          <button @click="$emit('openSettings')" class="px-4 py-2 text-sm text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">
            ⚙️ {{ $t('common.settings') }}
          </button>
        </div>
      </div>
      ...
      <div class="grid grid-cols-4 gap-3 mb-4">
        <div v-for="idx in indexList" :key="idx.code" class="bg-white rounded-xl p-3 shadow-sm border border-slate-100">
          <div class="text-xs text-slate-500">{{ idx.name }}</div>
          <div class="flex items-baseline gap-2">
            <span class="text-lg font-bold" :class="getIndexClass(idx.change_percent)">{{ idx.price }}</span>
      ...
      <!-- 错误提示 -->
      <div v-if="errorMsg" class="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg mb-4 text-sm">{{ errorMsg }}</div>
      ...
      <div class="flex items-center justify-between mb-3">
        <div class="flex items-center gap-2">
          ...
          <button @click="currentGroup = ''"
            :class="!currentGroup ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'"
            class="px-3 py-1 text-xs rounded-full transition-colors">
            全部
          </button>
          <button v-for="g in groupList" :key="g" 
            @click="currentGroup = g"
            @contextmenu="showGroupContextMenu($event, g)"
            :class="currentGroup === g ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'"
            class="px-3 py-1 text-xs rounded-full transition-colors">
            {{ g }}
          </button>
          <button @click="showAddGroupModal = true" class="px-2 py-1 text-xs text-blue-500 border border-blue-200 rounded-full hover:bg-blue-50">+ 新建</button>
        </div>
        <div class="flex items-center gap-2">
          <span class="text-sm text-slate-500">排序:</span>
          <select v-model="sortBy" class="text-xs border border-slate-200 rounded px-2 py-1 focus:outline-none">
        ...
        <table class="w-full table-fixed">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-100">
              ...
              <th class="w-24 px-2 py-3 text-left text-xs font-semibold text-slate-500">代码</th>
              <th class="w-20 px-2 py-3 text-left text-xs font-semibold text-slate-500">名称</th>
              <th class="w-20 px-2 py-3 text-right text-xs font-semibold text-slate-500">当前价</th>
              <th class="w-20 px-2 py-3 text-right text-xs font-semibold text-slate-500 cursor-pointer hover:text-blue-500" @click="toggleSort">涨跌幅 {{ sortIcon }}</th>
              <th class="w-16 px-2 py-3 text-right text-xs font-semibold text-slate-500">最高</th>
              <th class="w-16 px-2 py-3 text-right text-xs font-semibold text-slate-500">最低</th>
              <th class="w-20 px-2 py-3 text-right text-xs font-semibold text-slate-500">成交额</th>
              <th class="w-16 px-2 py-3 text-left text-xs font-semibold text-slate-500">分组</th>
              <th class="w-28 px-2 py-3 text-center text-xs font-semibold text-slate-500">操作</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100">
            <tr v-for="(stock, index) in filteredStocks" :key="stock.code" 
              ...
              <td class="px-1 py-3 cursor-move" @click.stop>
                <span class="text-slate-400 hover:text-slate-600 text-sm font-bold">⋮⋮</span>
              </td>
              ...
              <td class="px-2 py-3 text-sm text-right font-semibold tabular-nums" :class="getPriceClass(stock.change_percent)">{{ stock.price }}</td>
              ...
              <td class="px-2 py-3 text-xs text-slate-500">
                <span v-if="stockGroups[stock.code]" class="px-1.5 py-0.5 bg-blue-50 text-blue-600 rounded">{{ stockGroups[stock.code] }}</span>
              </td>
    ...
    <!-- 分组右键菜单 -->
    <div v-if="groupContextMenu.show" class="fixed bg-white rounded-lg shadow-xl border border-slate-200 py-1 z-50 min-w-40"
      :style="{ left: groupContextMenu.x + 'px', top: groupContextMenu.y + 'px' }" @click.stop>
      <div class="px-4 py-2 text-xs text-slate-400 border-b border-slate-100">{{ groupContextMenu.group }}</div>
      <button @click="handleDeleteGroup(false)" class="w-full px-4 py-2 text-left text-sm hover:bg-slate-50">🗑️ 删除分组</button>
      <button @click="handleDeleteGroup(true)" class="w-full px-4 py-2 text-left text-sm text-red-500 hover:bg-red-50">⚠️ 删除分组及股票</button>
    </div>

    <!-- 右键菜单 -->
    <div v-if="contextMenu.show" class="fixed bg-white rounded-lg shadow-xl border border-slate-200 py-1 z-50 min-w-32"
      :style="{ left: contextMenu.x + 'px', top: contextMenu.y + 'px' }" @click.stop>
      <button @click="handleContextAction('top')" class="w-full px-4 py-2 text-left text-sm hover:bg-slate-50">📌 置顶</button>
      <button @click="handleContextAction('bottom')" class="w-full px-4 py-2 text-left text-sm hover:bg-slate-50">📍 置底</button>
      <div class="border-t border-slate-100 my-1"></div>
      <div class="px-4 py-2 text-xs text-slate-400">移动到分组</div>
      <button @click="handleContextAction('group', '')" class="w-full px-4 py-2 text-left text-sm hover:bg-slate-50">无分组</button>
      <button v-for="g in groupList" :key="g" @click="handleContextAction('group', g)" class="w-full px-4 py-2 text-left text-sm hover:bg-slate-50">
        {{ g }} <span v-if="stockGroups[contextMenu.stock?.code] === g" class="text-blue-500">✓</span>
      </button>
      <button @click="handleContextAction('newGroup')" class="w-full px-4 py-2 text-left text-sm text-blue-500 hover:bg-blue-50">+ 新建分组</button>
      <div class="border-t border-slate-100 my-1"></div>
      <button @click="handleContextAction('delete')" class="w-full px-4 py-2 text-left text-sm text-red-500 hover:bg-red-50">🗑️ 删除</button>
    </div>
    ...
    <div v-if="showAlertModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="closeAlertModal">
      <div class="bg-white rounded-xl shadow-xl w-96 p-6">
        <h3 class="text-lg font-semibold text-slate-800 mb-4">预警设置 - {{ currentAlertStock?.name }}</h3>
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-slate-600 mb-1">止盈价格</label>
            <input v-model="alertForm.take_profit" type="number" step="0.01" placeholder="价格达到时提醒"
          ...
          <div>
            ...
            <input v-model="alertForm.stop_loss" type="number" step="0.01" placeholder="价格跌至时提醒"
              class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-600 mb-1">涨跌幅预警 (%)</label>
            <input v-model="alertForm.change_alert" type="number" step="0.1" placeholder="涨跌幅达到时提醒"
              class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
          <div class="flex items-center gap-2">
            <input v-model="alertForm.enabled" type="checkbox" id="alert-enabled" class="w-4 h-4" />
            <label for="alert-enabled" class="text-sm text-slate-600">启用预警</label>
          </div>
        </div>
        <div class="flex justify-end gap-3 mt-6">
          <button @click="closeAlertModal" class="px-4 py-2 text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">取消</button>
          <button @click="saveAlert" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">保存</button>
        </div>
      </div>
    </div>

    <!-- 新建分组弹窗 -->
    <div v-if="showAddGroupModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click.self="closeAddGroupModal">
      <div class="bg-white rounded-xl shadow-xl w-80 p-6">
        <h3 class="text-lg font-semibold text-slate-800 mb-4">新建分组</h3>
        <p v-if="pendingGroupStock" class="text-xs text-slate-500 mb-2">创建后将把当前股票移动到此分组</p>
        ...
        <div class="flex justify-end gap-3">
          <button @click="closeAddGroupModal" class="px-4 py-2 text-slate-600 border border-slate-200 rounded-lg hover:bg-slate-50">取消</button>
          <button @click="addGroup" class="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">确定</button>
        </div>
      </div>
    </div>
  </div>
...
const dismissAlert = (index: number) => { alertNotifications.value.splice(index, 1) }
